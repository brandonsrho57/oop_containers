...
class Heap(BinaryTree):
    ...
    def __init__(self, xs=None):
        ...
        then each element of xs needs to be inserted
        into the Heap.
        '''
        super().__init__()
        if xs is not None:
    ...
    def __repr__(self):
        ...
        return a string that can be used to recreate
        a valid instance of the class.
        ...
        the string "Heap",
        but for the AVLTree, this expression will be
        "AVLTree".
        Using this expression ensures that all
        subclasses of Heap will have a correct
    ...
    def insert(self, value):
        ...
        binary representation of the total number of nodes
            ...
            of your heap in a variable (rather than compute
            it) to maintain the O(log n) runtime
            1. See https://stackoverflow.com/questions/
            18241192/implement-heap-using-a-binary-tree for
            hints
    ...
    def remove_min(self):
        ...
        2. replace the root node with what was formerly
        the bottom right
        3. "trickle down" the root node: recursively swap
        it with its largest child until the heap property
        is satisfied
    ...
    @staticmethod
    def _remove_bottom_right(node, rmpath):
        rmvalue = ""
        if len(rmpath) == 0:
            return None, None
        if rmpath[0] == '0':
            if len(rmpath) == 1:
                rmvalue = node.left.value
                node.left = None
            else:
                rmvalue, node.left = Heap._remove_bottom_right(node.left, rmpath[1:])
        if rmpath[0] == '1':
            if len(rmpath) == 1:
                rmvalue = node.right.value
                node.right = None
            else:
                rmvalue, node.right = Heap._remove_bottom_right(node.right, rmpath[1:])
        print(rmvalue, str(node))
        return rmvalue, node
