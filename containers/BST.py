'''
This file implements the Binary Search Tree
data structure.
...
class BST(BinaryTree):
    ...
    def __repr__(self):
        ...
        For the BST, type(self).__name__ will be the
        string "BST",
    ...
    def __eq__(self, t2):
        ...
        '''
        ils = self.to_list('inorder')
        ils2 = t2.to_list('inorder')
        return ils == ils2
    ...
    def _is_bst_satisfied(node):
        ...
        You should use the _ prefixed methods
        because those are static methods just like
        this one.
        '''
        ret = True
        if node.left:
    ...
    def _remove(value, node):
        ...
        if node.value == value:
            if node.left is None:
                ...
                right_minimum = BST._find_smallest(node.right)
                node.value = right_minimum
                node.right = BST._remove(right_minimum, node.right)
                return node
